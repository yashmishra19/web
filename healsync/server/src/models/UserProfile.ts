...
export interface IUserProfile extends Document {
  ...
  mainGoal: 'better_sleep'|'reduce_stress'|
            'fitness'|'better_routine'|'improve_focus'
...
const UserProfileSchema = new Schema<IUserProfile>({
  ...
  dietPreference: {
    type: String, required: true,
    enum: ['omnivore','vegetarian','vegan','keto','other'],
  },
  stressLevel:    { type: Number, required: true, min: 1, max: 5 },
  moodBaseline:   { type: Number, required: true, min: 1, max: 5 },
  workStudyHours: { type: Number, required: true, min: 0, max: 24 },
  mainGoal: {
    type: String, required: true,
    enum: ['better_sleep','reduce_stress',
           'fitness','better_routine','improve_focus'],
  },
